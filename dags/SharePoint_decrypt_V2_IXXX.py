...
dag = DAG(
dag_id='SharePoint_decrypt_V2_IXXX',
...
def init(objects, **kwargs):
     iconfig = Variable.get("SharePoint_decrypt_V2_IXXX_config", deserialize_json=True)
...
@dag.task(task_id="decrypt_decrypt_oebs_gbl")
def decrypt_decrypt_oebs_gbl(local_path, vars, **context):
     logDebug(f'Dag : Encrypting files downloaded to {local_path} ')
     local_path, error = AmGlPGPOperator (
                ...
                public_key_id=vars['decrypt_oebs_gbl']['public_key_id'],
                private_key_path=vars['decrypt_oebs_gbl']['private_key_path'],
                private_key_password=vars['decrypt_oebs_gbl']['private_key_password'],
                decrypted_file_extension=vars['decrypt_oebs_gbl']['decrypted_file_extension'],
...
local_path = decrypt_decrypt_oebs_gbl(local_path, vars)
