...
class SchemaRegistryClient(object):
    ...
    def lookup_schema(self, subject_name, schema):
        ...
        """  # noqa: E501
        request = {'schema': schema.schema_str}
        print(f"schema_str - {schema.schema_str}")
        # CP 5.5 adds new fields (for JSON and Protobuf).
        if len(schema.references) > 0 or schema.schema_type != 'AVRO':
            request['schemaType'] = schema.schema_type
            request['references'] = [{'name': ref.name,
        ...
        URL = 'subjects/{}'.format(_urlencode(subject_name))
        URL = URL+'/versions/latest'
        print(f"URL - {URL}")
        response = self._rest_client.get(URL,None)
