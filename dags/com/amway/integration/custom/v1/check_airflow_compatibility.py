...
- Breaking changes
- Make it compatible with Airflow 3.0
```python
{code}
```"""
...
def main():
    ...
    # Scan all .py files under DAGs and plugins
    paths = glob.glob("gcs/amGlGCSCleanup.py", recursive=True)

    print(f"Found {len(paths)} Python files to analyze.")

    for path in paths:
        print(f"Analyzing: {path}")
        result = analyze_code(path)
        with open(f"{report_dir}/{os.path.basename(path)}.report.txt", "w", encoding="utf-8") as f:
            f.write(result)
        print(result)
