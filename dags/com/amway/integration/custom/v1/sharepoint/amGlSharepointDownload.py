...
class AmGlSharepointDownload(BaseOperator):
    ...
    template_fields = ('conn_id', 'regex', 'local_path', 'local_path_exits', 'remote_url', 'team_site_url', 'instance_id')
    ...
    def execute(self, context: Any) -> str:
        ...
        try:
            is_downloaded = False
            logDebug(f'Operator : called module with {self.conn_id}, {self.remote_url}, {self.regex}, {self.team_site_url} and {self.local_path}')
            ...
            #filtering the files based on regex
            ...
            if is_downloaded == False:
                message = f'Operator: AmGISharepointDownload no files to download'
            else:
                message = f'Operator: AmGISharepointDownload files {filtered_files} downloaded to '+ path 
