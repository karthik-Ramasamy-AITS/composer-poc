...
class AmGlGCSDownload(BaseOperator):
    ...
    template_fields = ('conn_id' , 'remote_path', 'file_filter', 'local_path', 'local_path_exits','instance_id', 'bucket_name', 'continue_on_failure')
    ...
    def execute(self, context: Any) -> str:
        error = None
        try:
            logDebug(f'Operator : called module with {self.conn_id}, {self.remote_path}, {self.file_filter}, {self.local_path}, {self.local_path_exits}, {self.instance_id}, {self.continue_on_failure} and {self.bucket_name}')
            is_downloaded = False
            ...
            filtered_files, error = filter_files(split_filename_files, self.file_filter)
            ...
            logToPVF(os.environ.get('AIRFLOW_CTX_DAG_RUN_ID'), #key
                     ...
                     f'Operator: Failure in AmGlGCSDownload, reason {e}, Retry count - {os.environ["AIRFLOW_CTX_TRY_NUMBER"]}' #ActivityMessage
                     )
            status = handleFailures(self.continue_on_failure)
            if status is True:
                logInfo(f'Operator: Failure in AmGlGCSDownload, reason {e}, as continue_on_failure is True')
            else:
