    ...
    def execute(self, context: Any) -> str:
        ...
        try:
                    ...
                    if os.path.isfile(self.local_path+actual):
                        if self.transfer_empty_files is True:
                            ...
                            if Path(self.local_path+actual).stat().st_size > 0:
                                ...
                                logInfo(f'skipping file {actual} as file-size is 0kb')
            ...
            if is_transferred is True and self.transfer_empty_files is True:
                ActivityMessage = f'Operator: AmGlGCSUpload files {files} updated to {self.remote_path}'
            elif is_transferred is True and self.transfer_empty_files is False:
                ActivityMessage = f'Operator: AmGlGCSUpload files {files} updated to {self.remote_path} and skipped {empty_files} files size is 0kb'
            else:
                ActivityMessage = f'Operator: AmGlGCSUpload - skipping {empty_files} files size is 0kb'
            ...
            logToPVF(os.environ.get('AIRFLOW_CTX_DAG_RUN_ID'), #key
                     ...
                     'ERROR',
                     self.conn_id,#targetApplicationCode
                     f'Operator: Failure in AmGlGCSUpload, reason {e}, Retry count - {os.environ["AIRFLOW_CTX_TRY_NUMBER"]}' #ActivityMessage
                     )
            status = handleFailures(self.continue_on_failure)
            if status is True:
                logInfo(f'Operator: Failure in AmGlGCSUpload, reason {e}, as continue_on_failure is True')
            else:
