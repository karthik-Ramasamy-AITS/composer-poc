...
class AmGlHTTPOperator(BaseOperator):
    ...
    template_fields = ('conn_id' , 'method', 'request_url', 'data', 'headers', 'instance_id', 'continue_on_failure')
    ...
    def execute(self, context: Any) -> str:
        ...
        try:
            ...
            logToPVF(os.environ.get('AIRFLOW_CTX_DAG_RUN_ID'), #key
                     ...
                     'ERROR',
                     self.conn_id,#targetApplicationCode
                     f'Operator: Failure in AmGlHTTPOperator, reason {e}, Retry count - {os.environ["AIRFLOW_CTX_TRY_NUMBER"]}' #ActivityMessage
                     )
            status = handleFailures(self.continue_on_failure)
            if status is True:
                logInfo(f'Operator: Failure calling AmGlHTTPOperator, reason {e}, as continue_on_failure is True')
            else:
