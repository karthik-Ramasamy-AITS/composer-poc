    ...
    def execute(self, context: Any) -> str:
        ...
        try:
            ...
            logToPVF(os.environ.get('AIRFLOW_CTX_DAG_RUN_ID'), #key
                     ...
                     f'Operator: Failure in AmGlS3Download, reason {e}, Retry count - {os.environ["AIRFLOW_CTX_TRY_NUMBER"]}' #ActivityMessage
                     )
            status = handleFailures(self.continue_on_failure)
            if status is True:
                logInfo(f'Operator: Failure in AmGlS3Download, reason {e}, as continue_on_failure is True')
            else:
