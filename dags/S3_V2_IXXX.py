...
default_args={'owner': 'gicoe', 'retries': 3, 'retry_delay': datetime.timedelta(seconds=2), 'email_on_failure': True, 'email': ['mahendra.ulchapogu@amway.com'], 'email_on_retry': False,'on_failure_callback': failure_callback}
dag = DAG(
dag_id='S3_V2_IXXX',
...
def init(objects, **kwargs):
     iconfig = Variable.get("S3_V2_IXXX_config", deserialize_json=True)
...
@dag.task(task_id="download_from_s3_inbound")
def download_from_s3_inbound(vars, **kwargs):
     logDebug(f'Dag : Dag config is {vars} ')
     if vars['dag_run_filter'] == None:
        file_filter = vars['s3_inbound']['file_filter']
     else:
        file_filter = vars['dag_run_filter']
     local_path, error = AmGlS3Download(
                    task_id = generateRandom(),
                    conn_id=vars['s3_inbound']['connection'],
                    remote_path=vars['s3_inbound']['source_path'],
                    file_filter=file_filter,
                    local_path=vars['local_path'],
                    local_path_exits=False,
                    instance_id=vars['instance_id'],
                    bucket_name=vars['s3_inbound']['bucket_name'],
                    continue_on_failure=vars['s3_inbound']['continue_on_failure']
                ).execute(None)
     logInfo(f'Dag : Matched from Dag files are {local_path}')
     return local_path


@dag.task(task_id="upload_s3_outbound")
def upload_s3_outbound(local_path, vars, **kwargs):
     logDebug(f'Dag : local path is {local_path} ')
     status, error = AmGlS3Upload (
                task_id=generateRandom(),
                conn_id=vars['s3_outbound']['connection'],
                local_path=local_path,
                regex=vars['s3_outbound']['regex'],
                replacement=vars['s3_outbound']['replacement'],
                rename_mask=vars['s3_outbound']['rename_mask'],
                remote_path=vars['s3_outbound']['target_path'],
                instance_id=vars['instance_id'],
                bucket_name=vars['s3_outbound']['bucket_name'],
                transfer_empty_files=vars['s3_outbound']['transfer_empty_files'],
                continue_on_failure=vars['s3_outbound']['continue_on_failure']
            ).execute(None)
     return status

@dag.task(task_id="archive_s3_inbound")
def archive_s3_inbound(local_path, status, vars, **kwargs):
     logDebug(f'Dag : local path is {local_path} ')
     status, error = AmGlS3Upload (
                task_id=generateRandom(),
                conn_id=vars['s3_inbound']['connection'],
                local_path=local_path,
                regex=vars['s3_inbound']['regex'],
                replacement=vars['s3_inbound']['replacement'],
                rename_mask=vars['s3_inbound']['rename_mask'],
                remote_path=vars['s3_inbound']['archive_path'],
                instance_id=vars['instance_id'],
                bucket_name=vars['s3_inbound']['bucket_name'],
                transfer_empty_files=vars['s3_inbound']['transfer_empty_files']
            ).execute(None)
     return status

@dag.task(task_id="cleanup_s3_inbound")
def cleanup_s3_inbound(status, vars, **kwargs):
     logInfo(f'Dag : Dag config is {vars} ')
     download_path = f'{vars["local_path"]}{os.environ.get("AIRFLOW_CTX_DAG_ID")}/{vars["instance_id"]}/'
     if "bucket_name" in vars['s3_inbound']:
          bucket_name = vars['s3_inbound']['bucket_name']
     else:
          bucket_name = None
     status, error = AmGlCleanup(
                    ...
                    remote_path=vars['s3_inbound']['source_path'],
                    file_filter=vars['s3_inbound']['file_filter'],
                    delete_sources=vars['s3_inbound']['delete_sources'],
                    type='s3',
                    instance_id=vars['instance_id'],
...
vars = init(None)
local_path = download_from_s3_inbound(vars)
status = upload_s3_outbound(local_path, vars)
status = archive_s3_inbound(local_path,status,vars)
status = cleanup_s3_inbound(status, vars)
