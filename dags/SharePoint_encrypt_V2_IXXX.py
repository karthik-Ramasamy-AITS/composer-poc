...
dag = DAG(
dag_id='SharePoint_encrypt_V2_IXXX',
...
def init(objects, **kwargs):
     iconfig = Variable.get("SharePoint_encrypt_V2_IXXX_config", deserialize_json=True)
...
@dag.task(task_id="encrypt_encrypt_oebs_gbl")
def encrypt_encrypt_oebs_gbl(local_path, vars, **context):
     ...
     local_path, error = AmGlPGPOperator (
                ...
                instance_id=vars['instance_id'],
                public_key_path=vars['encrypt_oebs_gbl']['public_key_path'],
                public_key_id=None,
                private_key_path=vars['encrypt_oebs_gbl']['private_key_path'],
                private_key_password=vars['encrypt_oebs_gbl']['private_key_password'],
                with_compression=vars['encrypt_oebs_gbl']['with_compression'],
                with_signing=vars['encrypt_oebs_gbl']['with_signing'],
                decrypted_file_extension=None,
                encrypted_file_extension=vars['encrypt_oebs_gbl']['encrypted_file_extension']
            ).execute(context)
     return local_path
...
local_path = encrypt_encrypt_oebs_gbl(local_path, vars)
