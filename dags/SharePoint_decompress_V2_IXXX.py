...
dag = DAG(
dag_id='SharePoint_decompress_V2_IXXX',
...
def init(objects, **kwargs):
     iconfig = Variable.get("SharePoint_decompress_V2_IXXX_config", deserialize_json=True)
...
@dag.task(task_id="decompress_oebs_gbl_decompress")
def decompress_oebs_gbl_decompress(local_path, vars, **context):
     logDebug(f'Dag : Decompressing files to {local_path} ')
...
local_path = decompress_oebs_gbl_decompress(local_path, vars)
