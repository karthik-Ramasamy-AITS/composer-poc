...
default_args={'owner': 'oebs', 'retries': 3, 'retry_delay': datetime.timedelta(seconds=120), 'email_on_failure': True, 'email': [], 'email_on_retry': False,'on_failure_callback': failure_callback}
dag = DAG(
dag_id='FTP_V2_IXXXX',
...
def init(objects, **kwargs):
     iconfig = Variable.get("FTP_v2_IXXX_config", deserialize_json=True)
...
@dag.task(task_id="download_from_ftp_inbound")
def download_from_ftp_inbound(vars, **kwargs):
     logDebug(f'Dag : Dag config is {vars} ')
     if vars['dag_run_filter'] == None:
        file_filter = vars['ftp_inbound']['file_filter']
     else:
        file_filter = vars['dag_run_filter']
     local_path, error = AmGlFTPDownload(
                    task_id = generateRandom(),
                    conn_id=vars['ftp_inbound']['connection'],
                    remote_path=vars['ftp_inbound']['source_path'],
                    file_filter=file_filter,
                    local_path=vars['local_path'],
                    local_path_exits=False,
                    instance_id=vars['instance_id'],
                    continue_on_failure=vars['ftp_inbound']['continue_on_failure']
                ).execute(None)
     logInfo(f'Dag : Matched from Dag files are {local_path}')
     return local_path
@dag.task(task_id="followup_download_from_ftp_source")
def followup_download_from_ftp_source(local_path, vars, **kwargs):
     logDebug(f'Dag : Dag config is {vars} ')
     if vars['dag_run_filter'] == None:
        file_filter = vars['ftp_source']['file_filter']
     else:
        file_filter = vars['dag_run_filter']
     local_path, error = AmGlFTPDownload(
                    task_id = generateRandom(),
                    conn_id=vars['ftp_source']['connection'],
                    remote_path=vars['ftp_source']['source_path'],
                    file_filter=file_filter,
                    local_path=local_path,
                    local_path_exits=True,
                    instance_id=vars['instance_id'],
                    continue_on_failure=vars['ftp_source']['continue_on_failure']
                ).execute(None)
     logInfo(f'Dag : Matched from Dag files are {local_path}')
     return local_path


@dag.task(task_id="upload_ftp_outbound")
def upload_ftp_outbound(local_path, vars, **kwargs):
     logDebug(f'Dag : local path is {local_path} ')
     status, error = AmGlFTPUpload (
                task_id=generateRandom(),
                conn_id=vars['ftp_outbound']['connection'],
                local_path=local_path,
                regex=vars['ftp_outbound']['regex'],
                replacement=vars['ftp_outbound']['replacement'],
                rename_mask=vars['ftp_outbound']['rename_mask'],
                remote_path=vars['ftp_outbound']['target_path'],
                instance_id=vars['instance_id'],
                transfer_empty_files=vars['ftp_outbound']['transfer_empty_files'],
                continue_on_failure=vars['ftp_outbound']['continue_on_failure']
            ).execute(None)
     return status
@dag.task(task_id="upload_followup_ftp_target")
def upload_followup_ftp_target(local_path, status, vars, **kwargs):
     logDebug(f'Dag : local path is {local_path} ')
     status, error = AmGlFTPUpload (
                task_id=generateRandom(),
                conn_id=vars['ftp_target']['connection'],
                local_path=local_path,
                regex=vars['ftp_target']['regex'],
                replacement=vars['ftp_target']['replacement'],
                rename_mask=vars['ftp_target']['rename_mask'],
                remote_path=vars['ftp_target']['target_path'],
                instance_id=vars['instance_id'],
                transfer_empty_files=vars['ftp_target']['transfer_empty_files'],
                continue_on_failure=vars['ftp_target']['continue_on_failure']
            ).execute(None)
     return status

@dag.task(task_id="archive_ftp_inbound")
def archive_ftp_inbound(local_path, status, vars, **kwargs):
     logDebug(f'Dag : local path is {local_path} ')
     status, error = AmGlFTPUpload (
                task_id=generateRandom(),
                conn_id=vars['ftp_inbound']['connection'],
                local_path=local_path,
                regex=vars['ftp_inbound']['regex'],
                replacement=vars['ftp_inbound']['replacement'],
                rename_mask=vars['ftp_inbound']['rename_mask'],
                remote_path=vars['ftp_inbound']['archive_path'],
                instance_id=vars['instance_id'],
                transfer_empty_files=vars['ftp_inbound']['transfer_empty_files']
            ).execute(None)
     return status
@dag.task(task_id="archive_ftp_source")
def archive_ftp_source(local_path, status, vars, **kwargs):
     logDebug(f'Dag : local path is {local_path} ')
     status, error = AmGlFTPUpload (
                task_id=generateRandom(),
                conn_id=vars['ftp_source']['connection'],
                local_path=local_path,
                regex=vars['ftp_source']['regex'],
                replacement=vars['ftp_source']['replacement'],
                rename_mask=vars['ftp_source']['rename_mask'],
                remote_path=vars['ftp_source']['archive_path'],
                instance_id=vars['instance_id'],
                transfer_empty_files=vars['ftp_source']['transfer_empty_files']
            ).execute(None)
     return status

@dag.task(task_id="cleanup_ftp_inbound")
def cleanup_ftp_inbound(status, vars, **kwargs):
     logInfo(f'Dag : Dag config is {vars} ')
     download_path = f'{vars["local_path"]}{os.environ.get("AIRFLOW_CTX_DAG_ID")}/{vars["instance_id"]}/'
     if "bucket_name" in vars['ftp_inbound']:
          bucket_name = vars['ftp_inbound']['bucket_name']
     else:
          bucket_name = None
     status, error = AmGlCleanup(
                    ...
                    remote_path=vars['ftp_inbound']['source_path'],
                    file_filter=vars['ftp_inbound']['file_filter'],
                    delete_sources=vars['ftp_inbound']['delete_sources'],
                    type='ftp',
                    instance_id=vars['instance_id'],
     ...
     return status
@dag.task(task_id="cleanup_ftp_source")
def cleanup_ftp_source(status, vars, **kwargs):
     logInfo(f'Dag : Dag config is {vars} ')
     download_path = f'{vars["local_path"]}{os.environ.get("AIRFLOW_CTX_DAG_ID")}/{vars["instance_id"]}/'
     if "bucket_name" in vars['ftp_source']:
          bucket_name = vars['ftp_source']['bucket_name']
     else:
          bucket_name = None
     status, error = AmGlCleanup(
                    task_id = generateRandom(),
                    ...
                    remote_path=vars['ftp_source']['source_path'],
                    file_filter=vars['ftp_source']['file_filter'],
                    delete_sources=vars['ftp_source']['delete_sources'],
                    type='ftp',
                    instance_id=vars['instance_id'],
                    bucket_name=bucket_name,
                    local_path=download_path
...
vars = init(None)
local_path = download_from_ftp_inbound(vars)
local_path = followup_download_from_ftp_source(local_path, vars)
status = upload_ftp_outbound(local_path, vars)
status = upload_followup_ftp_target(local_path,status,vars)
status = archive_ftp_inbound(local_path,status,vars)
status = archive_ftp_source(local_path,status,vars)
status = cleanup_ftp_inbound(status, vars)
status = cleanup_ftp_source(status, vars)
